...
export default function App() {
  ...
  const [selectedCharacter, setSelectedCharacter] = useState<'RYU' | 'KEN' | 'CHUN_LI' | 'AKUMA'>('RYU');
  const [startingRoute, setStartingRoute] = useState<'BEACH' | 'FOREST' | 'SUMMIT'>('BEACH');
  ...
  const [isBannerAlert, setIsBannerAlert] = useState<boolean>(false);

  // Trigger brief alert flash when a new wave begins
  useEffect(() => {
    if (stage === 'PLAYING') {
      setIsBannerAlert(true);
  ...
  const startGame = () => {
    ...
    let startWave = 1;
    let initialZombieType: 'groomer' | 'screamer' | 'brute' | 'boss' = 'groomer';
    if (startingRoute === 'FOREST') {
      startWave = 6;
      initialZombieType = 'screamer';
    } else if (startingRoute === 'SUMMIT') {
      startWave = 11;
      initialZombieType = 'brute';
    }
    ...
    lastKeyPressTimeRef.current = Date.now();
    setZombieType(initialZombieType);
    setRyuAction('idle');
        ...
        <div className="flex-1 relative flex flex-col overflow-hidden bg-zinc-950">
          ...
          {stage === 'TITLE' && (
            <div className="flex-1 flex flex-col items-center justify-between py-4 px-4 relative overflow-y-auto bg-gradient-to-b from-[#060b11] via-[#0d1420] to-[#070b12] w-full h-full">
              ...
              {/* 統合ゲームコントロールパネル (Character, Route, and Difficulty Select) */}
              <div className="z-10 flex flex-col gap-3.5 w-full max-w-xl bg-black/60 p-4 rounded-lg border border-zinc-900 shadow-2xl shrink-0">
                
                {/* 1. キャラクター選択 (FIGHTER SELECT) */}
                <div className="flex flex-col items-center">
                  <span className="font-press-start text-[8px] text-zinc-500 tracking-wider mb-2 uppercase leading-none">
                    SELECT FIGHTER:
                  </span>
                  <div className="grid grid-cols-4 gap-2 w-full text-[9px] font-press-start">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedCharacter('RYU');
                        setPlayerNameInput('RYU');
                        audio.playType();
                      }}
                      className={`py-2 px-1 rounded border-2 transition-all cursor-pointer flex flex-col items-center gap-1 ${
                        selectedCharacter === 'RYU'
                          ? 'bg-neutral-850 border-white text-white shadow-[0_0_12px_rgba(255,255,255,0.15)] font-bold'
                          : 'bg-zinc-950/60 border-zinc-900 text-zinc-500 hover:border-zinc-800'
                      }`}
                    >
                      <div className="w-4 h-4 rounded-full bg-neutral-200 border border-neutral-400 flex items-center justify-center font-press-start text-[6.5px] text-black">R</div>
                      <span>RYU</span>
                    </button>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedCharacter('KEN');
                        setPlayerNameInput('KEN');
                        audio.playType();
                      }}
                      className={`py-2 px-1 rounded border-2 transition-all cursor-pointer flex flex-col items-center gap-1 ${
                        selectedCharacter === 'KEN'
                          ? 'bg-red-950/80 border-red-500 text-red-400 shadow-[0_0_12px_rgba(239,68,68,0.2)] font-bold'
                          : 'bg-zinc-950/60 border-zinc-900 text-zinc-500 hover:border-zinc-800'
                      }`}
                    >
                      <div className="w-4 h-4 rounded-full bg-red-600 border border-red-400 flex items-center justify-center font-press-start text-[6.5px] text-yellow-300">K</div>
                      <span>KEN</span>
                    </button>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedCharacter('CHUN_LI');
                        setPlayerNameInput('CHUN');
                        audio.playType();
                      }}
                      className={`py-2 px-1 rounded border-2 transition-all cursor-pointer flex flex-col items-center gap-1 ${
                        selectedCharacter === 'CHUN_LI'
                          ? 'bg-blue-950/80 border-blue-500 text-blue-400 shadow-[0_0_12px_rgba(59,130,246,0.2)] font-bold'
                          : 'bg-zinc-950/60 border-zinc-900 text-zinc-500 hover:border-zinc-800'
                      }`}
                    >
                      <div className="w-4 h-4 rounded-full bg-blue-600 border border-blue-400 flex items-center justify-center font-press-start text-[6.5px] text-white">C</div>
                      <span>CHUN</span>
                    </button>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedCharacter('AKUMA');
                        setPlayerNameInput('AKUMA');
                        audio.playType();
                      }}
                      className={`py-2 px-1 rounded border-2 transition-all cursor-pointer flex flex-col items-center gap-1 ${
                        selectedCharacter === 'AKUMA'
                          ? 'bg-purple-950/80 border-purple-600 text-purple-400 shadow-[0_0_12px_rgba(168,85,247,0.25)] font-bold animate-pulse'
                          : 'bg-zinc-950/60 border-zinc-900 text-zinc-500 hover:border-zinc-800'
                      }`}
                    >
                      <div className="w-4 h-4 rounded-full bg-purple-900 border border-purple-700 flex items-center justify-center font-press-start text-[6.5px] text-red-500">A</div>
                      <span>AKUMA</span>
                    </button>
                  </div>
                  <div className="text-[8.5px] text-zinc-400 mt-1.5 font-mono text-center">
                    {selectedCharacter === 'RYU' && "🥋 [RYU] - 波動拳と昇竜拳の王道バランスファイター"}
                    {selectedCharacter === 'KEN' && "🔥 [KEN] - 炎を纏う連撃のスピードファイター"}
                    {selectedCharacter === 'CHUN_LI' && "⚡ [CHUN-LI] - 超高速キックを操る華麗なスピードスター"}
                    {selectedCharacter === 'AKUMA' && "👹 [AKUMA] - 殺意の波動で全てを滅する隠し怒りキャラクター"}
                  </div>
                </div>

                {/* 2. 攻略開始ルート選択 (ROUTE SELECT) */}
                <div className="flex flex-col items-center border-t border-zinc-900 pt-3">
                  <span className="font-press-start text-[8px] text-zinc-500 tracking-wider mb-2 uppercase leading-none">
                    SELECT START ROUTE:
                  </span>
                  <div className="grid grid-cols-3 gap-2 w-full text-[9px] font-press-start">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setStartingRoute('BEACH');
                        audio.playType();
                      }}
                      className={`py-2 px-1 rounded border-2 transition-all cursor-pointer ${
                        startingRoute === 'BEACH'
                          ? 'bg-cyan-950/60 border-cyan-500 text-cyan-400 shadow-[0_0_8px_rgba(6,182,212,0.25)] font-bold'
                          : 'bg-zinc-950/60 border-zinc-900 text-zinc-500 hover:border-zinc-800'
                      }`}
                    >
                      BEACH (W1)
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setStartingRoute('FOREST');
                        audio.playType();
                      }}
                      className={`py-2 px-1 rounded border-2 transition-all cursor-pointer ${
                        startingRoute === 'FOREST'
                          ? 'bg-emerald-950/60 border-emerald-500 text-emerald-400 shadow-[0_0_8px_rgba(16,185,129,0.25)] font-bold'
                          : 'bg-zinc-950/60 border-zinc-900 text-zinc-500 hover:border-zinc-800'
                      }`}
                    >
                      FOREST (W6)
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setStartingRoute('SUMMIT');
                        audio.playType();
                      }}
                      className={`py-2 px-1 rounded border-2 transition-all cursor-pointer ${
                        startingRoute === 'SUMMIT'
                          ? 'bg-amber-950/60 border-amber-600 text-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.25)] font-bold'
                          : 'bg-zinc-950/60 border-zinc-900 text-zinc-500 hover:border-zinc-800'
                      }`}
                    >
                      SUMMIT (W11)
                  ...
                  <div className="text-[8.5px] text-zinc-400 mt-1.5 font-mono text-center">
                    {startingRoute === 'BEACH' && "🏖️ 海岸 (Wave 1〜5) - 無線アンテナ回収までのチュートリアル区画"}
                    {startingRoute === 'FOREST' && "🌲 暗い森 (Wave 6〜10) - 俊敏なゾンビが襲来する中間区画"}
                    {startingRoute === 'SUMMIT' && "⛰️ 山頂通信塔 (Wave 11〜15) - 救助ヘリを待つ地獄の最終防衛圏"}
                  </div>
                </div>

                {/* 3. 難易度選択 (DIFFICULTY SELECT) */}
                <div className="flex flex-col items-center border-t border-zinc-900 pt-3">
                  <span className="font-press-start text-[8px] text-zinc-500 tracking-wider mb-2 uppercase leading-none">
                    SELECT DIFFICULTY:
                  </span>
                  ...
                  <div className="text-[8.5px] text-zinc-400 mt-1.5 font-mono text-center">
                    {difficulty === 'EASY' && "🟢 タイマー制限1.35倍 & ゾンビHPが通常より30%低減"}
                    {difficulty === 'NORMAL' && "🟡 スタンダードな標準タイマー速度 & 基準ゾンビHP"}
                    {difficulty === 'HARD' && "🔴 タイマー制限0.75倍の電撃戦 & ゾンビHPが40%増加"}
                  </div>
                </div>
          ...
          {stage === 'PLAYING' && (
            <div className="flex-1 h-full min-h-0 flex flex-col gap-2 md:gap-3 justify-between select-none p-1 sm:p-2">
              ...
              <div id="island-quest-banner" className={`border pixel-border p-3 rounded flex flex-col md:flex-row justify-between items-start md:items-center gap-2.5 z-10 backdrop-blur-sm shrink-0 transition-all duration-300 ${
                isBannerAlert 
                  ? 'animate-banner-alert' 
                  : `bg-[#0c121e]/90 ${areaInfo.borderColor} shadow-[0_4px_20px_rgba(0,0,0,0.6)]`
              }`}>
                <div className="flex items-center gap-3">
              ...
              <div className={`col-span-12 rounded-lg relative overflow-hidden flex-1 min-h-[200px] md:min-h-[250px] flex flex-col justify-end transition-all duration-500 pixel-border bg-gradient-to-b ${areaInfo.bgHex} ${isBossActive ? 'border-red-600 shadow-[inset_0_0_35px_rgba(239,68,68,0.3)]' : 'border-zinc-850 shadow-[inset_0_0_20px_rgba(0,0,0,0.85)]'}`}>
                ...
                <div className="flex-grow flex items-end justify-between px-8 pb-1 z-10 pointer-events-none select-none">
                  <RyuSprite action={ryuAction} comboCount={combo} character={selectedCharacter} />
